...
{
    //alert($write_pages . " // " . $cur_page . " // " . $total_page . " // "  . $url . " // " . $add);
...
//        $url .= substr($url, -1) === '?' ? 'start_date=' . $start_date . '?' : '&amp;start_date=' . $start_date . '?';
    }
...
//        $url .= substr($url, -1) === '?' ? 'end_date=' . $end_date . '?' : '&amp;end_date=' . $end_date . '?';
    }
...
//        $url .= substr($url, -1) === '?' ? 'assign=' . $assign . '?' : '&amp;assign=' . $assign . '?';
    }
    if ($sex != null && $sex != "") {
        if (substr($url, -1) === '?' || substr($url, -1) === '&') {
            $url .=  'sex=' . $sex ;
...
//        $url .= substr($url, -1) === '?' ? 'sex=' . $sex . '?' : '&amp;sex=' . $sex . '?';
    }
    if ($sido != null && $sido != "") {
        if (substr($url, -1) === '?' || substr($url, -1) === '&') {
            $url .=  'sido=' . $sido;
...
//        $url .= substr($url, -1) === '?' ? 'sido=' . $sido . '?' : '&amp;sido=' . $sido . '?';
    }
...
//        $url .= substr($url, -1) === '?' ? 'filter=' . $filter . '?' : '&amp;filter=' . $filter . '?';
    }
...
//        $url .= substr($url, -1) === '?' ? 'orderby=' . $orderby . '?' : '&amp;orderby=' . $orderby . '?';
    }
...
{
	//alert($mb_id."::".$point."::".$content."::".$rel_table."::".$rel_id."::".$rel_action."::".$expire);
    global $config;
    global $g5;
    global $is_admin;
